...
class LiteLLMAIHandler(BaseAiHandler):
    ...
    def __init__(self):
        ...
        if get_settings().get("GROQ.KEY", None):
            ...
            # [BlackboxTester] Support GROQ_API_KEY from environment (Docker)
            # MUST set litellm.api_key for Groq models to authenticate
    ...
    async def chat_completion(self, model: str, system: str, user: str, temperature: float = 0.2, img_path: str = None):
        try:
            ...
            # [BlackboxTester] Force Groq API Key if using Groq (Bypass OpenRouter overwrite)
            if model.startswith("groq/") and os.environ.get("GROQ_API_KEY"):
                # Unset OpenRouter envs that might confuse litellm
                if "OPENROUTER_API_KEY" in os.environ:
                    del os.environ["OPENROUTER_API_KEY"]
                if "OPENROUTER_API_BASE" in os.environ:
                    del os.environ["OPENROUTER_API_BASE"]
                
                groq_key = os.environ.get("GROQ_API_KEY", "").strip()
                if groq_key:
                    kwargs["api_key"] = groq_key
                    # [DEBUG] Verify key injection using Loguru to avoid console spam if not needed
                    get_logger().info(f"Injecting Groq API Key (length: {len(groq_key)}) for model {model}")
                
                # [BlackboxTester] IMPORTANT: Remove api_base to force litellm to use official Groq endpoint
                # If api_base is set (even to None/empty), it might confuse routing
                if "api_base" in kwargs:
                    del kwargs["api_base"]
                    get_logger().info("Removed api_base from kwargs for Groq model")

            # [DEBUG] Log kwargs keys just before call to verify api_key is present
            get_logger().debug(f"litellm.acompletion kwargs keys: {list(kwargs.keys())}")
        ...
        # Safe print to avoid Loguru crash on braces
        # print(f"\nAI response:\n{resp}") 
        # get_logger().debug(f"\nAI response:\n{resp}")
        pass
