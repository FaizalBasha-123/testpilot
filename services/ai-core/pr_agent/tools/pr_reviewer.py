...
class PRReviewer:
    ...
    async def run(self) -> None:
        try:
            ...
            if hasattr(self, 'sonar_review_body') and self.sonar_review_body:
                pr_review += "\n\n" + self.sonar_review_body
    ...
    async def _prepare_prediction(self, model: str) -> None:
        # [SonarQube Integration] - Run early to feed context to AI
        sonar_findings_str = ""
        if get_settings().get("sonarqube.enabled", False):
            try:
                from pr_agent.tools.pr_sonar_scan import PRSonarScan
                get_logger().info("Running SonarQube analysis for Auto-Fix context...")
                sonar_tool = PRSonarScan(self.pr_url, args=["--no_publish"])
                sonar_output = await sonar_tool.run()
                
                if sonar_output:
                    # Parse the output to extract just the issues for the prompt
                    # Logic assumes _format_findings returns a markdown string.
                    # We'll pass the whole string or a summary.
                    sonar_findings_str = f"\n\n🔍 **Static Analysis Findings**:\n{sonar_output}\n\n"
                    sonar_findings_str += "IMPORTANT: You MUST generate specific diff/code suggestions to FIX the static analysis issues listed above."
                    
                    # Store for later appending to output
                    self.sonar_review_body = sonar_output 
            except Exception as e:
                get_logger().error(f"Failed to run SonarQube analysis: {e}")
        ...
        if self.patches_diff:
            ...
            # Inject Sonar findings into the prompt vars
            if sonar_findings_str:
                current_instructions = self.vars.get("extra_instructions", "")
                self.vars["extra_instructions"] = f"{current_instructions}\n{sonar_findings_str}"
                get_logger().info("Injected SonarQube findings into AI prompt for auto-fix generation")
