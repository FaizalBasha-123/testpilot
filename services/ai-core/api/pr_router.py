...
async def handle_request(body: Dict[str, Any], event: str, delivery_id: str = ""):
    ...
    try:
        from pr_agent.payment.payment_manager import PaymentManager
        import json
        
        sender = body.get("sender", {}).get("login")
        pm = PaymentManager()
        
        # Simple scan for promo code in the entire payload
        # This allows users to include "UPCRAFT#123" in their PR description or comment to bypass
        payload_str = json.dumps(body)
        promo_code = "UPCRAFT#123" if "UPCRAFT#123" in payload_str else None
        
        if not pm.check_access(sender, promo_code):
            get_logger().warning(f"⚠️ Access denied for user '{sender}'. Production Mode: {pm.is_production}")
            return {}
    except Exception as e:
        get_logger().error(f"Payment check failed: {e}")
        # Fail safe: in production, deny access if check fails? Or allow?
        # For now, log and proceed (safe for MVP testing) or return to block.
        # "Reason yourself" -> Secure default is block.
        return {}
...
# [BlackboxTester] IDE API
from pr_agent.servers.ide_router import router as ide_router
